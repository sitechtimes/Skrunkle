...
(function (BABYLON) {
  var DynamicTerrain = /** @class */ (function () {
    ...
    function DynamicTerrain(name, options, scene) {
      ...
      this._terrain = BABYLON.MeshBuilder.CreateRibbon(
        "terrain",
        ribbonOptions,
        this._scene
      );
      ...
      this._normals = this._terrain.getVerticesData(
        BABYLON.VertexBuffer.NormalKind
      );
      this._uvs = this._terrain.getVerticesData(BABYLON.VertexBuffer.UVKind);
      this._colors = this._terrain.getVerticesData(
      ...
      if (this._mapInstanceData) {
        ...
        for (var t = 0; t < typeNb; t++) {
          ...
          if (this._colorInstanceData) {
            ...
            var colorBuffer = new BABYLON.VertexBuffer(
              engine,
              colorArray,
              BABYLON.VertexBuffer.ColorKind,
              true,
    ...
    DynamicTerrain.prototype._updateTerrain = function (updateSize) {
      ...
      BABYLON.Vector3.FromFloatsToRef(
        Number.MAX_VALUE,
        Number.MAX_VALUE,
        Number.MAX_VALUE,
        bbMin
                ...
                if (instanceIndexes && instancedBuffer) {
                  ...
                  for (var iIdx = 0; iIdx < min; iIdx++) {
                    ...
                    if (precomputeInstances) {
                      ...
                      x =
                        x +
                        Math.floor((terrainPos.x - x - x0) / mapSizeX) *
                          mapSizeX;
                      z =
                        z +
                        Math.floor((terrainPos.z - z - z0) / mapSizeZ) *
                          mapSizeZ;
     ...
     * If the optional object {normal: Vector3} is passed, then its property "normal" is updated with the normal vector value at the coordinates (x, z).
     */
    DynamicTerrain.GetHeightFromMap = function (
      x,
      z,
      mapData,
      mapSubX,
    ...
    DynamicTerrain._GetHeightFromMap = function (
      x,
      z,
      mapData,
      mapSubX,
     ...
     */
    DynamicTerrain.ComputeNormalsFromMapToRef = function (
      mapData,
      mapSubX,
      mapSubZ,
      normals,
    ...
    /**
         ...
         */
    DynamicTerrain.CreateMapFromHeightMap = function (
      heightmapURL,
      options,
      scene
    ) {
     ...
     */
    DynamicTerrain.CreateMapFromHeightMapToRef = function (
      heightmapURL,
      options,
      data,
      scene
     ...
     * @param target
     */
    DynamicTerrain._CopyArrayValuesFromToRef = function (
      source,
      start,
      nb,
      target
    ...
    DynamicTerrain._matZero = new Float32Array([
