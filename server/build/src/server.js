  ...
  SocketServer.prototype.listen = function () {
    ...
    this.server.on("connection", function (client) {
      ...
      if (!_this.world.players.has(client)) {
        ...
        _this.send(
          client,
          new packet_1.Packet(packet_1.PacketType.info, [
            {
              player: player,
        ...
        _this.send(
          client,
          new packet_1.Packet(packet_1.PacketType.mesh, _this.world.entities)
        );
      }
      client.on("message", function (message) {
        ...
        switch (msg.type) {
          case "Movement":
            ...
            if (player !== null) {
              player.position = _this.world.validateEntityPosition(
                new babylonjs_1.Vector3(
                  msg.payload[0].position._x,
                  msg.payload[0].position._y,
                  msg.payload[0].position._z
                )
              );
          ...
          case "Close":
            _this.broadCast(
              new packet_1.Packet(packet_1.PacketType.close, [
                { id: msg.uid, delete: true },
              ])
            );
            break;
          case "Interaction":
            ...
            _this.broadCast(
              new packet_1.Packet(
                packet_1.PacketType.interaction,
                msg.payload[0]
              )
            );
            break;
          case "Chat":
            ...
            _this.broadCast(
              new packet_1.Packet(packet_1.PacketType.chat, msg.payload[0])
            );
            break;
          case "ping":
            ...
            _this.send(
              client,
              new packet_1.Packet(packet_1.PacketType.info, ["Pong!"])
            );
            break;
          case "PlayerCreation":
            ...
            if (!_this.players.has(client)) {
              ...
              _this.send(
                client,
                player_2.serialize(packet_1.PacketType.player_creation, {
          ...
          case "RequestMesh":
            ...
            _this.send(
              client,
              (_a = state_machine_1.state_machine.entities.get(mesh_id)) ===
                null || _a === void 0
                ? void 0
                : _a.serialize()
            );
          ...
          default:
            _this.logger.error(
              "Unknown socket message from client (".concat(msg.type, ")")
            );
            break;
        }
      });
      client.on("close", function () {
        ...
        state_machine_1.state_machine.delete_player(
          _this.client_to_uid.get(client)
        );
